...
class SecurityManager:
    """Handles encryption, decryption, and authentication using Fernet symmetric encryption"""
    
    def __init__(self, password: str = "default_chat_password"):
        self.password = password.encode()
        self.key = self._derive_key()
        self.fernet = Fernet(self.key)
        self.auth_hash = self._derive_auth_hash()
    
    def _derive_key(self) -> bytes:
        """Derive a key from password using PBKDF2 for encryption"""
        salt = b'Z2dSt9xmZqeIh6Rwg1yIRUuRxMv6zSCr7PMR9EYfZyg='  # Encryption salt
        kdf = PBKDF2HMAC(
    ...
    def _derive_auth_hash(self) -> str:
        """Derive authentication hash from password using different salt"""
        auth_salt = b'AuthSalt2025ChatApp_SecureLogin_PBKDF2='  # Authentication salt
        kdf = PBKDF2HMAC(
            algorithm=hashes.SHA256(),
            length=32,
            salt=auth_salt,
        ...
        auth_key = kdf.derive(self.password)
        return base64.urlsafe_b64encode(auth_key).decode()
    
    def verify_password(self, provided_password: str) -> bool:
        """Verify if provided password matches the server password"""
        try:
            temp_manager = SecurityManager(provided_password)
            return temp_manager.auth_hash == self.auth_hash
        except Exception:
            return False
...
class AuthenticationManager:
    ...
    def __init__(self, security_manager: SecurityManager):
        ...
        self.authenticated_clients = set()  # Set of authenticated socket objects
        self._lock = threading.Lock()
    
    def authenticate_client(self, client_socket: Any, password: str) -> bool:
        """Authenticate a client with the provided password"""
    ...
    def is_authenticated(self, client_socket: Any) -> bool:
        """Check if client is authenticated"""
        with self._lock:
            return client_socket in self.authenticated_clients
    
    def logout_client(self, client_socket: Any):
        """Remove client from authenticated clients"""
        with self._lock:
            self.authenticated_clients.discard(client_socket)
